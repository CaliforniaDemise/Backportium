    ...
    protected static byte[] transformBlockLiquid(byte[] basicClass) {
        ...
        for (MethodNode method : cls.methods) {
            if (method.name.equals(getName("shouldSideBeRendered", "func_176225_a"))) {
                ...
                list.add(new MethodInsnNode(INVOKEVIRTUAL, "net/minecraft/util/math/BlockPos", getName("offset", "func_177972_a"), "(Lnet/minecraft/util/EnumFacing;)Lnet/minecraft/util/math/BlockPos;", false));
    ...
    protected static byte[] transformWorldEntitySpawner(byte[] basicClass) {
        ...
        for (MethodNode method : cls.methods) {
            if (method.name.equals(getName("isValidEmptySpawnBlock", "func_185331_a"))) {
                ...
                list.add(hook("fluidLogging$entitySpawning", "(ZLnet/minecraft/block/state/IBlockState;)Z"));
                method.instructions.insertBefore(node, list);
                break;
            }
        }
