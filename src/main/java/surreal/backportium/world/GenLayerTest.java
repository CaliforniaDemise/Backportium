...
public class GenLayerTest extends GenLayer {
    ...
    public static void register() {
        MinecraftForge.TERRAIN_GEN_BUS.register(Register.class);
    }

    public static class Register {
        @SubscribeEvent
        public static void initBiomes(WorldTypeEvent.InitBiomeGens event) {
            GenLayer[] layers = event.getNewBiomeGens();
            GenLayer[] replacement = new GenLayer[layers.length];
            for (int i = 0; i < layers.length; i++) {
                GenLayer test = new GenLayerTest(2L, layers[i]);
    ...
    public GenLayerTest(long seed, GenLayer parent) {
        super(seed);
        this.parent = parent;
    }
